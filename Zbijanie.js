...
GAME.komunikat = function(kom){
	if(this.koms.indexOf(kom)==-1){
		if(this.komc=60) this.komc=60;
		var ind=this.koms.push(kom)-1;
		JQS.kcc.append('<div class="kom" style="top:'+this.komc+'%"><div class="close_kom" data-ind="'+ind+'"><img src="/gfx/layout/exit.png" /></div><div class="content">'+kom+'</div></div>');
		this.komc++;
		kom_close_bind();
	}
}
 ...
 */
const CONF_MIN_PA = Math.floor(GAME.getCharMaxPr() / 100 * 0.4)
...
function handleResponse (res) {
	// on move response
	if (res.a === 4 && res.char_id === GAME.char_id && GAME.char_data.klan_id == 3542) setTimeout(() => {
	...
	// on fight response
	else if (res.a === 7 && GAME.char_data.klan_id == 3542) setTimeout(() => {
		// when in the cell are some mobs
		const mobs = areMobsOnField()
		if (mobs) {
			fight(mobs.mob_num)
	...
	else if (res.a === 12 && res.type === 14 && GAME.char_data.klan_id == 3542) move()
